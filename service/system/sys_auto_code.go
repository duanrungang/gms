...
import (
	...
	"gms/model/system"
	"gms/pkg"
	"gms/resource/autocode_template/subcontract"
	"go.uber.org/zap"
	"go/ast"
	"go/format"
	"go/parser"
...
func (autoCodeService *AutoCodeService) PreviewTemp(autoCode system.AutoCodeStruct) (map[string]string, error) {
	...
	// 写入文件前，先创建文件夹
	if err = pkg.CreateDir(needMkdir...); err != nil {
		return nil, err
	}
...
func (autoCodeService *AutoCodeService) CreateTemp(autoCode system.AutoCodeStruct, ids ...uint) (err error) {
	...
	if autoCode.AutoMoveFile { // 判断是否需要自动转移
		...
		// 判断目标文件是否都可以移动
		for _, value := range dataList {
			if pkg.FileExist(value.autoMoveFilePath) {
				return errors.New(fmt.Sprintf("目标文件已存在:%s\n", value.autoMoveFilePath))
			}
		}
		for _, value := range dataList { // 移动文件
			if err := pkg.FileMove(value.autoCodePath, value.autoMoveFilePath); err != nil {
				return err
			}
		}
		...
		if err = pkg.ZipFiles("./gms.zip", fileList, ".", "."); err != nil {
			return err
		}
	}
...
func injectionCode(structName string, bf *strings.Builder) error {
	for _, meta := range injectionPaths {
		...
		if err := pkg.AutoInjectionCode(meta.path, meta.funcName, code); err != nil {
			return err
		}
