...
func (userService *UserService) Register(u system.SysUser) (userInter system.SysUser, err error) {
	...
	// 否则 附加uuid密码hash加密 注册
	u.Password = pkg.BcryptHash(u.Password)
	u.UUID = uuid.NewV4()
...
func (userService *UserService) ChangePassword(u *system.SysUser, newPassword string) (userInter *system.SysUser, err error) {
	...
	user.Password = pkg.BcryptHash(newPassword)
...
//@description: 设置一个用户的角色
//@param: uuid uuid.UUID, authorityId string
//@return: err error
func (userService *UserService) SetUserAuthority(id uint, uuid uuid.UUID, authorityId uint) (err error) {
	assignErr := global.GVA_DB.Where("sys_user_id = ? AND sys_authority_authority_id = ?", id, authorityId).First(&system.SysAuthority{}).Error
...
func (userService *UserService) ResetPassword(ID uint) (err error) {
	err = global.GVA_DB.Model(&system.SysUser{}).Where("id = ?", ID).Update("password", pkg.BcryptHash("123456")).Error
	return err
}
