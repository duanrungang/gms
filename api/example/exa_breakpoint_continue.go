...
import (
	...
	"gms/model/example"
	exampleRes "gms/model/example/response"
	"gms/pkg"
	"go.uber.org/zap"
	"io/ioutil"
	"mime/multipart"
...
func (b *FileUploadAndDownloadApi) BreakpointContinue(c *gin.Context) {
	...
	pathC, err := pkg.BreakPointContinue(cen, fileName, chunkNumber, chunkTotal, fileMd5)
	if err != nil {
...
func (b *FileUploadAndDownloadApi) BreakpointContinueFinish(c *gin.Context) {
	fileMd5 := c.Query("fileMd5")
	fileName := c.Query("fileName")
	filePath, err := pkg.MakeFile(fileName, fileMd5)
	if err != nil {
